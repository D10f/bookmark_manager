    ...
    <Combobox :modelValue="modelValue" @update:modelValue="handleUpdate">
        ...
        <div class="relative mt-1">
            <div
                ...
                <ComboboxInput class="w-full border-none py-2 pl-3 pr-10 text-sm leading-5 text-gray-900 focus:ring-0"
                    :displayValue="(option: any) => option" @change="query = $event.target.value" />
                <ComboboxButton class="absolute inset-y-0 right-0 flex items-center pr-2">
                    <IconChevron class="fill-gray-400 w-5 h-5" aria-hidden="true" />
                </ComboboxButton>
            </div>
        </div>
        ...
        <TransitionRoot leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0"
            ...
            <ComboboxOptions
                ...
                <ComboboxOption v-if="queryOption && !filteredOptions.length" as="ul" :value="queryOption"
                    v-slot="{ active }">
                    <li class="relative cursor-default select-none py-2 pl-10 pr-4" :class="{
                        ...
                        <span class="absolute inset-y-0 left-0 flex items-center pl-3" :class="{
                            'text-white': active,
                            'text-teal-600': !active,
                        }">
                            <IconPlus class="h-5 w-5 text-amber-600" aria-hidden="true" />
                        </span>
                    </li>
                </ComboboxOption>
                ...
                <ComboboxOption v-for="option in filteredOptions" as="template" :key="option.value" :value="option"
                    ...
                    <li class="relative cursor-default select-none py-2 pl-10 pr-4" :class="{
                        ...
                        <span v-if="selected" class="absolute inset-y-0 left-0 flex items-center pl-3" :class="{
                            ...
                            <IconCheck class="h-5 w-5 text-amber-600" aria-hidden="true" />
                        </span>
                    </li>
                </ComboboxOption>
            </ComboboxOptions>
...
type ComboboxProps = {
    ...
    label?: string;
    createOption: (categoryName: string) => void;
};
...
function handleUpdate(selection: ComboboxOption) {
    emit("update:modelValue", selection.label);

    if (selection.value < 0) {
        props.createOption(selection.label);
    }
}
</script>
